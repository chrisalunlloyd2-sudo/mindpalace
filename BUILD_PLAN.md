...
| R4 | ~~`bookEditor.open()` never wired~~ **CORRECTED 2026-08-17**: `open()` IS wired at `GameEngine:490` — the earlier claim was a broken grep. Books fail to open because of R5 (index math returns wrong book / none) | See R5 | ✅ fixed by A3 |
| R5 | Raycast index math ≠ visual placement | `raycastBooks` mapped hit → `startIdx + row*(perWall/3) + b` assuming uniform row-major layout; `WorldBuilder` places books grouped BY LANGUAGE with divider slots and 15-book cap → mismatch, wrong book or none | ✅ FIXED (A3): `findBookInSights` now iterates `book.isPlaced()` books and ray-AABBs their real `getWorldPosition()`; nearest hit wins |
...
- **A1. ✅ PATCHED 2026-08-17** FontRenderer glyph guard (R1):** in `drawText`, clamp `c` to 32..126 (else `?`). Add `TextSanitizer.stripCR()` applied before drawing. Test: glyphIndex mapping for `\r`, `\n`, `╔`, `é` all → safe char.
- **A2. ✅ PATCHED 2026-08-17** TextSanitizer (R2):** new `util/TextSanitizer.java`: `stripCR` (`\r\n`→`\n`, drop bare `\r`), `asciiSafe` (map box-drawing `╔║══╠╣╦╩╬─│┌┐└┘├┤┬┴•·` → `+-=|*`, non-ASCII → `?`). Apply in: `GitHubClient.fetchFileContent`, `Book` content set, `RepoMapper` repo names, door labels, HUD, `BookEditor` buffer. Test: CRLF sample → clean.
- **A3. ✅ PATCHED 2026-08-17** Wire book open (R4+R5):** in `GameEngine` left-click handler, pass the picked book to `bookEditor.open(...)`; fix `raycastBooks` to use `book.getWorldPosition()` AABB list instead of index math. Test: unit-test raycast against a placed book's world position.
- **A4. ✅ PATCHED 2026-08-17** Editor text in-world (R3):** render the terminal buffer onto the panel with FontRenderer billboard mode (lines wrapped to PANEL_WIDTH), scrollable. Keep System.out as a mirror. Test: terminal buffer → drawn line count matches.
- **A5. ✅ PATCHED 2026-08-17** Real qwen suggestions (R6):** `actionSuggest()` → build prompt (file path + language + first 200 lines + "suggest concrete code improvements"), call `OllamaClient.chat(ModelConfig.CODE_MODEL, ...)`, show numbered suggestions; `:a <n>` inserts at cursor. Test: mock OllamaClient; prompt structure assertion; insert path.
